...
object AppPrefs {
    ...
    const val KEY_SCHEDULE_VIEW_MODE = "schedule_view_mode" // "cards" or "full"
    ...
    fun getScheduleViewMode(ctx: Context): String =
        sp(ctx).getString(KEY_SCHEDULE_VIEW_MODE, "cards") ?: "cards"

    fun setScheduleViewMode(ctx: Context, mode: String) {
        require(mode == "cards" || mode == "full")
        sp(ctx).edit().putString(KEY_SCHEDULE_VIEW_MODE, mode).apply()
    }
